...
func insertUser(user models.Customer) int64 {
	...
	sqlStatement := `INSERT INTO customers (id, name) VALUES ($1, $2) RETURNING id`
	...
	err := db.QueryRow(sqlStatement, user.ID, user.Name).Scan(&id)

	if err != nil {
		log.Fatalf("Unable to execute the query. %v", err)
	}

	fmt.Printf("Inserted a single record %v", name)
...
func getUser(id int64) (models.Customer, error) {
	// create the postgres db connection
	db := createConnection()
	...
	sqlStatement := `SELECT * FROM customers WHERE userid=$1`
